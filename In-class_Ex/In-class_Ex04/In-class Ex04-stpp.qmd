...
## Spatio-temporal Point Patterns Analysis: stopp methods

::: callout-note
Remember to include stopp package in `p_load()`.
:::
...
Step 2: Converting the acq_date into numric data format.  Note that the base number starts from Jan 1, 1970.

```{r}
fire_sf <- fire_sf %>%
  mutate(numeric_date <- as.numeric(acq_date))
```

Step 3: Build a data.frame from the coordinates and date

```{r}
fire_df <- data.frame(
  x = coords[, 1],  
  y = coords[, 2],
  date = fire_sf$`DayofYear`)
```
...
fire_df <- data.frame(
  x = coords[, 1],  
  y = coords[, 2],
  date = fire_sf$`numeric_date <- as.numeric(acq_date)`)
```

Step 4: Creating a stp object

```{r}
fire_stp <- stp(fire_df)
```

Step 5: Plotting the s datapatio-temporal

```{r}
plot(fire_stp, tcum = TRUE)
```

```{r}
spatial_bandwidth <- seq(0, 5000, length.out = 100) 
temporal_bandwidth <- seq(0, 100, length.out = 50)  
```

```{r}
kst_result <- Kst(fire_stp, 
                  srange = spatial_bandwidth, 
                  trange = temporal_bandwidth)
```
...
```{r}
spatial_data <- as.data.frame(st_coordinates(fire_stp$geom))
```

```{r}
fire_stpp <- as.3dpoints(fire_list)
```
