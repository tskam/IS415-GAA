...
In this section, you will learn how to compute a distance matrix by using `distance()` of **SpatialAcc** package.

Firstly, `st_coordinates()` of sf package will be used to extract the coordinates from the *eldercare* and *hexagon* sf data frames. 
...
The code chunk below will be used to compute the distance matrix. 
...
Lastly, the code chunk below will be used to convert the unit of measurement from meter to kilometer.
...
hexagon_KD2SFCA <- bind_cols(hexagon, acc_KD2SFCA)
```
...
hexagon_SAM <- bind_cols(hexagon, acc_SAM)
```
...
DOMatrix <- ODMatrix %>%
  select(origin_id, destination_id, total_cost) %>%
  spread(origin_id, total_cost)%>%
  select(c(-c('destination_id')))
```

```{r}
DOMatrix_km <- as.matrix(DOMatrix/1000)
```

```{r}
sfca <- spai_3sfca(hexagon$demand,
                   eldercare$capacity,
                   DOMatrix_km,
                   step = 3)
```
...
```{r}
hexagon_sfca <- bind_cols(hexagon, sfca) %>%
  rename(Acc3SFCA = step3)
```

### Visualising 3SFCA accessibility

The code chunk below uses a collection of mapping fucntions of tmap package to create a high cartographic quality accessibility to eldercare centre in Singapore. Notice that `mapex` is reused for *bbox* argument.

```{r fig.width=12, fig.height=8}
tmap_mode("plot")
tm_shape(hexagon_sfca,
         bbox = mapex) + 
  tm_fill(col = "Acc3SFCA",
          n = 10,
          style = "quantile",
          border.col = "black",
          border.lwd = 1) +
