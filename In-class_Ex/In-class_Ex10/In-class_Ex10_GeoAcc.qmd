...
Firstly, `st_coordinates()` of sf package will be used to extract the coordinates from the *eldercare* sf data frames.  

```{r}
eldercare_coord <- st_coordinates(eldercare)
```

However, hexagon sf data frame is in polygon feature st_centroid() will be used to compute the centroid of each hexagon.  Then, st_coordinate() will be used to extract the coordinates from the newly derived centroid points.
